  ...
  d3.rebind(_orbitLayout, orbitDispatch, "on");
	...
	function calculateNodes() {
		...
		//otherwise assume it is an object with a root node
			...
			traverseNestedData(orbitNodes, 1)

		function traverseNestedData(_node, orbitIndex) {
			if(childrenAccessor(_node)) {
				...
				orbitalRings.push({source: _node, x: _node.x, y: _node.y, r: (_node.ring / 2) + (orbitIndex * 40)});
				...
				for (var x = 0; x<childrenAccessor(_node).length;x++) {
					...
					traverseNestedData(childrenAccessor(_node)[x], orbitIndex + 1);
				}
			}
		}
	}
